...
@when(u'CANBUS上剩余油量在{duration}秒内从{init_oil}升下降到{end_oil}升')
def step_impl(context, duration, init_oil, end_oil):
    context.dorothyTestInput.sysExtEvt.set_initial_value('Speed', float(init_oil))
    context.dorothyTestInput.sysExtEvt.set_end_value('Speed', float(end_oil))
...
@when(u'CANBUS上剩余油量在{duration}秒内从{init_oil}升上升到{end_oil}升')
def step_impl(context, duration, init_oil, end_oil):
    context.dorothyTestInput.sysExtEvt.set_initial_value('Speed', int(init_oil))
    context.dorothyTestInput.sysExtEvt.set_end_value('Speed', int(end_oil))
    context.dorothyTestInput.sysExtEvt.set_signal_duration(int(duration))
    context.dorothyTestInput.sysExtEvt.start_generate_signal()


@then(u'CANBUS上剩余油量在{duration}秒内从{init_oil}升下降到{end_oil}升')
def step_impl(context, duration, init_oil, end_oil):
    context.dorothyExpRes.set_value('oil', int(end_oil))
    context.dorothyActRes.mock_value('oil', int(end_oil))
    context.dorothyActRes.get_value('oil')
    logging.debug("context.dorothyExpRes.Speed: " + str(context.dorothyExpRes.dist['oil']))
    logging.debug("context.dorothyActRes.Speed: " + str(context.dorothyActRes.dist['oil']))
    assert context.dorothyActRes.dist['oil'] == context.dorothyExpRes.dist['oil']