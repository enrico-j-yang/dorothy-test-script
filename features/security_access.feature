...
功能: 安全访问
  ...
  场景:    安全访问请求级别1密钥请求超出密钥访问次数限制，延时时间后请求种子成功，请求密钥错误
    ...
    当      CANBUS上等待10秒后
    而且    CANBUS上有以物理寻址方式的安全访问请求，参数为请求级别1种子
    而且    CANBUS上信号发送1次
  ...
  场景:    从安全访问状态解锁1复位，延时时间内请求种子失败，延时时间后请求种子成功，请求密钥成功
    ...
    当      CANBUS上有以物理寻址方式的安全访问请求，参数SecurityAccessType为级别1密钥，参数SecurityKey为有效值
    而且    CANBUS上信号发送1次
  ...
  场景:    从安全访问状态解锁1进入编程会话模式，请求种子成功，请求密钥成功
    ...
    当      CANBUS上有以物理寻址方式的诊断会话控制请求，参数为进入编程会话
    而且    CANBUS上信号发送一次
  ...
  场景:    从安全访问状态解锁2复位，延时时间内请求种子失败，延时时间后请求种子成功，请求密钥成功
    ...
    当      CANBUS上有以物理寻址方式的安全访问请求，参数为请求级别2种子
    而且    CANBUS上信号发送1次

    那么    CANBUS上有安全访问否定响应，参数NegativeResponseCode为延迟时间未到

    当      CANBUS上等待10秒后
    而且    CANBUS上有以物理寻址方式的安全访问请求，参数为请求级别2种子
    而且    CANBUS上信号发送1次
