...
功能: 读取DTC信息
  ...
  场景:    读取 DTC 信息 – 通过状态掩码报告所有ISO15031-6DTCFormat格式 DTC 数目成功
    ...
    假如    HUD上DTC信息的数目为1
    当      CANBUS上有读取DTC信息请求，参数Sub-Fucntion为通过状态掩码报告DTC数目，参数DTCStatusMask为0xFF
    而且    CANBUS上信号发送一次
  ...
  场景:    读取 DTC 信息 – 通过状态掩码报告单个 DTC 数目成功
    假如    HUD上有ISO15031-6DTCFormat格式DTC信息
    假如    HUD上DTC信息的数目为1
    当      CANBUS上有读取DTC信息请求，参数Sub-Fucntion为通过状态掩码报告DTC数目，参数DTCStatusMask为0x01
    而且    CANBUS上信号发送一次
  ...
  场景:    读取 DTC 信息 – 通过0x00状态掩码报告 DTC 数目成功
    假如    HUD上有ISO15031-6DTCFormat格式DTC信息
    假如    HUD上DTC信息的数目为1
    当      CANBUS上有读取DTC信息请求，参数Sub-Fucntion为通过状态掩码报告DTC数目，参数DTCStatusMask为0x00
    而且    CANBUS上信号发送一次
  ...
  场景:    读取 DTC 信息 – 通过状态掩码报告 DTC成功
    假如    HUD上有ISO15031-6DTCFormat格式DTC信息
    假如    HUD上DTC信息的数目为1
    当      CANBUS上有读取DTC信息请求，参数Sub-Fucntion为通过状态掩码报告 DTC，参数DTCStatusMask为0xFF
    而且    CANBUS上信号发送一次
